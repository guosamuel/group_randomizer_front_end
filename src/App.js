...
import SavedGroupContainer from "./containers/SavedGroupsContainer/SavedGroupsContainer";
...
function App(props) {
  ...
  const randomizeGroups = (number, choices) => {
    ...
    if (isGroups) {
      ...
      let totalNumberOfGroups = Math.floor(
        copyOfOptions.length / convertedToIntegerNumber
      );
  ...
  return (
    <div className="App">
      ...
      <br />
      <button
        onClick={(number, choices) =>
          randomizeGroups(props.number, props.options)
        }
        disabled={
          props.options.length === 0 ||
          !props.number ||
          parseInt(props.number, 10) <= 0
            ? true
            : false
        }
        style={{
      ...
      <button
        ...
        disabled={
          (props.options.length === 0 && props.randomizedGroups.length === 0) ||
          !props.number ||
          parseInt(props.number, 10) <= 0
            ? true
            : false
      ...
      {props.randomizedOption.length !== 0 ? (
        <div>
          ...
          <p
            className={
              (props.randomizedOption.includes(" ")
                ? "not-one-long-word"
                : "one-long-word") + " padding-center"
            }
          >
            {props.randomizedOption}
          </p>
        </div>
      ) : null}
      {props.randomizedOrder.length !== 0 ? (
        <div>
          ...
          <ol className="padding-center">{randomizedOrderList}</ol>
        </div>
      ) : null}
      <SavedGroupContainer />
    </div>
